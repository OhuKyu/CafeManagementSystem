            ...
            else
            {
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        string selectUsername = "SELECT * FROM users WHERE username = @usern";//Dùng để kiểm tra user đó đã được sử dụng hay chưa
                        ...
                        using (SqlCommand checkUsername = new SqlCommand(selectUsername, connect))
                        {
                            ...
                            {
                                string usern = register_username.Text.Substring(0, 1).ToUpper() + register_password.Text.Substring(1);
                            ...
                            {
                                MessageBox.Show("Mật khẩu không phù hợp, phải ít nhất 8 ký tự!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            ...
                            else
                            {
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertData, connect))
                                {
                                    ...
                                    cmd.Parameters.AddWithValue("@role", "Thu ngân");
                    ...
                    {
                        MessageBox.Show("Kết nối không thành công!" + ex, "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    finally
                    {
                        connect.Close();
