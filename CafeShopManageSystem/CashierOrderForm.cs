...
namespace CafeShopManageSystem
{
    public partial class CashierOrderForm : UserControl
    {
        ...
        {
            using (SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\DP221405\cafe.mdf;Integrated Security=True;Connect Timeout=30"))
            {
                connect.Open();
        ...
        private void cashierOrderForm_amount_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show("Không hợp lệ á!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cashierOrderForm_amount.Text = "";
                    cashierOrderForm_change.Text = "";
                }
            }
        ...
        private void cashierOrderForm_payBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Sai dữ liệu nào đó, Vui lòng kiểm tra lại.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (MessageBox.Show("Bạn chắc chắn là khách đã trả tiền?", "Xác nhận"
                    ...
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertData, connect))
                            {
                                ...
                                MessageBox.Show("Trả tiền xong r đó biến đi hộ!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            string headerText = "Quán Cafe của Duy ó ;3 ";
            ...
            e.Graphics.DrawString("Tổng tiền: \t$" + totalPrice + "\nSố tiền: \t$"
                + cashierOrderForm_amount.Text + "\n\t\t------------\nTiền thừa: \t$" + cashierOrderForm_change.Text, labelFont, Brushes.Black, labelX, y);
        ...
        private void cashierOrderForm_removeBtn_Click(object sender, EventArgs e)
        {
            if (getOrderID == 0)
            {
                MessageBox.Show("Bạn chưa chọn món nào!", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                if (MessageBox.Show("Bạn chắc chắn muốn xoá đơn hàng có ID là: " + getOrderID + "?", "Xác nhận"
                , MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        private void cashierOrderForm_orderTable_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                getOrderID = Convert.ToInt32(row.Cells["id"].Value);
            }
        }
